...
export default function NotFoundPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setParticles(prev => prev.map(p => ({
        ...p, x: p.x + p.vx, y: p.y + p.vy, vy: p.vy + 0.05, opacity: p.opacity - 0.025,
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#09090b] flex items-center justify-center px-5 relative overflow-hidden cursor-crosshair select-none">
      {particles.map(p => (
        <div key={p.id} className="absolute rounded-full bg-white pointer-events-none" style={{ left: p.x, top: p.y, width: p.size, height: p.size, opacity: p.opacity * 0.4 }} />
      ))}
      ...
      <div className="text-center relative z-10 transition-transform duration-100 ease-out" style={{ transform: `perspective(800px) rotateX(${tiltX}deg) rotateY(${tiltY}deg)` }}>
        <div className="text-[120px] sm:text-[140px] font-black text-white/5 mb-1 tracking-tighter leading-none">404</div>
